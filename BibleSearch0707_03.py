...
class BibleApp:
    ...
    def search(self):
        ...
        if book in book_files:
            ...
            current_chapter = 0
            for line in lines:
                if line.strip().startswith(str(current_chapter + 1) + ":1"):
                    current_chapter += 1
                ...
                if len(parts) > 1:
                    chapter, verse_num = map(int, parts[0].split(':'))
                    if (current_chapter == start_chapter and verse_num >= start_verse) or \
                       (current_chapter == end_chapter and verse_num <= end_verse) or \
                       (start_chapter < current_chapter < end_chapter):
                        result += line
